...
// 【练习题】
// ================================================================
// 1. 在代码中添加多行注释和单行注释，说明 `#define` 和 `constexpr` 的区别
// 2. 编写一个程序，使用 `#define` 定义宏计算两数之和，再用 constexpr 实现相同功能，比较两者差异
// 3. 写一个程序用条件编译 `#if/#else/#endif` 实现 debug 和 release 两种编译模式
// 4. 将程序参数 argc/argv 的用法封装为一个函数，打印所有传入参数
// 5. 研究 C++17 `if (init; condition)` 语法，编写示例验证其作用域控制效果
